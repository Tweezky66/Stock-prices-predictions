...
# 6. VWAP (Distance from the VWAP as a percentage)
...
df['Target_Future_Return'] = (df['Close'].shift(-21) - df['Close']) / df['Close']

df.dropna(subset=['MACD_Norm', 'RSI', 'ATR_Pct'], inplace=True)
...
model_vol = XGBRegressor(n_estimators=100, max_depth=5, random_state=42)
model_vol.fit(X_train, y_vol_train) 

model_ret = XGBRegressor(n_estimators=100, max_depth=5, random_state=42)
...
for i, day in enumerate(days):
    expected_price = current_price * (1 + predicted_daily_drift * day)
